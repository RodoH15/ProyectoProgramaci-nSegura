    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if 'codigo_archivo' in request.FILES:
                ...
                os.system(f'docker cp {archivo_path} programacion-executor-1:/code/submissions/')
...
def evaluar_con_casos_directos(code_path, casos_de_prueba):
    ...
    comando = f"docker exec programacion-executor-1 python3 /code/programacion/scripts.py {code_path} '{casos_de_prueba}'"
    result = subprocess.run(comando, shell=True, capture_output=True, text=True)
    print("stdout:", result.stdout)  # Agregar depuración de la salida estándar
    print("stderr:", result.stderr)  # Agregar depuración de la salida de error
    
    # Verificar si la salida estándar está vacía
    if result.stdout.strip() == "":
        raise Exception(f"Error in evaluating script. Stderr: {result.stderr.strip()}")
...
def calcular_puntaje(result):
    ...
    correct_cases = sum(1 for x in result if x is True)
    puntaje = (correct_cases / total_cases) * 100
    return puntaje
