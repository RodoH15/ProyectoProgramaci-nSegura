...
def crear_ejercicio(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ejercicio = form.save()
            # Guardar los casos de prueba en el contenedor executor
            casos_de_prueba_path = os.path.join('/code/test_case', f'casos_de_prueba_{ejercicio.id}.txt')
            # Copiar los casos de prueba al contenedor executor
            with open(casos_de_prueba_path, 'w') as f:
                f.write(ejercicio.casos_de_prueba)
            os.system(f'docker cp {casos_de_prueba_path} programacion_executor_1:/code/test_case/')
