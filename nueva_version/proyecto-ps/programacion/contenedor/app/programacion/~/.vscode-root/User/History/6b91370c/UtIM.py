...
def subir_respuesta(request, ejercicio_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            respuesta.nombre_alumno = usuario.username  # Asigna el nombre del alumno logueado
            ...
            # Evaluar el código subido usando los casos de prueba del ejercicio
            if respuesta.codigo_archivo:
                code_path = str(respuesta.codigo_archivo)
                test_cases_path = os.path.join('/tmp', 'casos_de_prueba_tmp.txt')  # Ruta temporal para los casos de prueba

                # Guardar los casos de prueba en un archivo temporal
                ...
                respuesta.puntaje = calcular_puntaje(result)
                respuesta.save()
            ...
            return redirect('detalle_ejercicio', ejercicio_id=ejercicio.id, puntaje=respuesta.puntaje)
    else:
        form = RespuestaEjercicioForm(initial={'nombre_alumno': usuario.username})
