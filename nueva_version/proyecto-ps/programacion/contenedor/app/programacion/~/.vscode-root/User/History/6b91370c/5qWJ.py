    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if respuesta.codigo_archivo:
                ...
                test_cases = casos_de_prueba.strip().split("\n\n")  # Separar casos de prueba en una lista
                print(f"Evaluando el archivo de código en: {code_path}")
                print(f"Usando los casos de prueba: {test_cases}")

                # Crear archivo temporal de casos de prueba en el contenedor executor
                test_cases_path = '/code/test_case/tmp_casos_de_prueba.txt'
                with open(test_cases_path, 'w') as f:
                    for case in test_cases:
                        f.write(case + '\n')

                # Copiar el archivo temporal al contenedor executor
                os.system(f'docker cp {test_cases_path} programacion-executor-1:/code/test_case/')
