...
def subir_respuesta(request, ejercicio_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                archivo_respuesta = request.FILES['codigo_archivo']
                casos_de_prueba = ejercicio.casos_de_prueba

                # Guardar el archivo de respuesta temporalmente
                with open('/tmp/' + archivo_respuesta.name, 'wb+') as destination:
                    for chunk in archivo_respuesta.chunks():
                        destination.write(chunk)

                resultados = evaluar('/tmp/' + archivo_respuesta.name, casos_de_prueba.path)

                # Calcular el puntaje basado en los resultados de la evaluación
                puntaje = sum(1 for resultado in resultados if resultado is True) / len(resultados) * 100
    ...
    return render(request, 'subir_respuesta.html', {'form': form, 'ejercicio': ejercicio, 'casos_de_prueba': ejercicio.casos_de_prueba.read()})
