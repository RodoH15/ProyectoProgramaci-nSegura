...
# views.py (parte del código relevante)
@login_required
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if 'codigo_archivo' in request.FILES:
                ...
                archivo_path = f'/tmp/{archivo.name}'  # Guardar el archivo temporalmente en /tmp
                with open(archivo_path, 'wb+') as destination:
                    for chunk in archivo.chunks():
                        destination.write(chunk)
                
                # Copiar el archivo al contenedor executor
                os.system(f'docker cp {archivo_path} programacion-executor-1:/code/submissions/{archivo.name}')
                ...
                # Analizar el código en el contenedor executor
                command_analizar = f"docker exec programacion-executor-1 python3 /code/programacion/scripts/analizar_codigo.py {code_path}"
                result_analizar = subprocess.run(command_analizar, shell=True, capture_output=True, text=True)
                if result_analizar.returncode != 0:
                    messages.error(request, f'Archivo no permitido: {result_analizar.stderr}')
                    return render(request, 'subir_respuesta.html', {'form': form, 'ejercicio': ejercicio, 'casos_de_prueba': casos_de_prueba})

            respuesta.codigo_archivo = code_path
            respuesta.save()
...
def evaluar_con_casos_directos(code_path, casos_de_prueba):
    # Ejecutar el contenedor executor con el código y los casos de prueba pasados directamente
    comando = f"docker exec programacion-executor-1 python3 /code/programacion/scripts/scripts.py {code_path} '{casos_de_prueba}'"
