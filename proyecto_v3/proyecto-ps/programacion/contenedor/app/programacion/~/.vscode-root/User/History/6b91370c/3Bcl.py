...
def subir_respuesta(request, ejercicio_id):
    ...
    casos_de_prueba_path = f'/code/test_case/casos_de_prueba_{ejercicio.id}.txt'  # Ruta en el contenedor executor
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Escribir los casos de prueba directamente en el contenedor executor
            result = os.system(f'docker exec programacion_executor_1 bash -c "echo \'{casos_de_prueba}\' > {casos_de_prueba_path}"')
            if result != 0:
                messages.error(request, 'Error al escribir los casos de prueba en el contenedor executor.')
                return render(request, 'subir_respuesta.html', {'form': form, 'ejercicio': ejercicio, 'casos_de_prueba': casos_de_prueba})
            ...
            if respuesta.codigo_archivo:
                ...
                test_cases_path = f'/code/test_case/casos_de_prueba_{ejercicio.id}.txt'  # Ruta en el contenedor executor
                print(f"Evaluando el archivo de código en: {code_path}")
                print(f"Usando los casos de prueba en: {test_cases_path}")

                # Asegurarse de que el archivo de casos de prueba existe en el contenedor executor
                check_file_cmd = f'docker exec programacion_executor_1 ls {test_cases_path}'
                if os.system(check_file_cmd) != 0:
                    messages.error(request, 'El archivo de casos de prueba no se encuentra en el contenedor executor.')
                    return render(request, 'subir_respuesta.html', {'form': form, 'ejercicio': ejercicio, 'casos_de_prueba': casos_de_prueba})

                result = evaluar(code_path, test_cases_path)
        ...
        form = RespuestaEjercicioForm(user=request.user)  # Llena el campo con el nombre del usuario logueado
    return render(request, 'subir_respuesta.html', {'form': form, 'ejercicio': ejercicio, 'casos_de_prueba': casos_de_prueba})
