...
def subir_respuesta(request, ejercicio_id):
    ejercicio = get_object_or_404(Ejercicio, id=ejercicio_id)
    puntaje = None  # Inicializamos puntaje en None
    if request.method == 'POST':
        form = RespuestaEjercicioForm(request.POST, request.FILES)
        if form.is_valid():
            ...
            if respuesta.codigo_archivo:
                code_path = str(respuesta.codigo_archivo)  # Asegúrate de pasar la ruta del archivo como una cadena
                test_cases_path = os.path.join('/code/test_case', 'casos_de_prueba.txt')  # Ruta a tus casos de prueba
                ...
                respuesta.save()
                puntaje = respuesta.puntaje  # Guardamos el puntaje para mostrarlo en la página
                print(f"Puntaje asignado: {respuesta.puntaje}")

            messages.success(request, f'La respuesta se ha subido exitosamente. Puntaje obtenido: {puntaje}')
            return render(request, 'subir_respuesta.html', {'form': form, 'ejercicio': ejercicio, 'puntaje': puntaje, 'casos_de_prueba': ejercicio.casos_de_prueba})
    else:
        form = RespuestaEjercicioForm(initial={'nombre_alumno': request.user.username})
    
    return render(request, 'subir_respuesta.html', {'form': form, 'ejercicio': ejercicio, 'casos_de_prueba': ejercicio.casos_de_prueba})
