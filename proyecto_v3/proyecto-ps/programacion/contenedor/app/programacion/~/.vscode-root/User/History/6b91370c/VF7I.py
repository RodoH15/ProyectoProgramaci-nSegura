    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if 'codigo_archivo' in request.FILES:
                ...
                archivo_path = os.path.join('/tmp', archivo.name)  # Guardar temporalmente en /tmp
                ...
                os.remove(archivo_path)  # Eliminar el archivo temporal
...
def evaluar_con_casos_directos(code_path, casos_de_prueba):
    ...
    print(f"Comando de evaluación: {comando}")
    result = subprocess.run(comando, shell=True, capture_output=True, text=True)
    print(f"Salida del comando de evaluación: {result.stdout}")
    if result.stderr:
        print(f"Errores del comando de evaluación: {result.stderr}")
