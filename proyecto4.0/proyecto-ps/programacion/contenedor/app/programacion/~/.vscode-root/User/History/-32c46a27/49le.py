...
def procesar_tarea(task_id, code_path, casos_path, respuesta_id):
    try:
        ...
        logger.info(f"Tarea {task_id} procesada exitosamente. Puntaje: {puntaje}")
    except Exception as e:
...
def worker():
    ...
    while not task_queue.empty():
        task_id, code_path, casos_path, respuesta_id = task_queue.get()
        procesar_tarea(task_id, code_path, casos_path, respuesta_id)
...
def add_task_to_queue(code_path, casos_path, respuesta_id):
    ...
    task_queue.put((task_id, code_path, casos_path, respuesta_id))
    if not processing:
        processing = True
