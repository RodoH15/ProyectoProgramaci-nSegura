...
def procesar_tarea(code_path, casos_path, respuesta_id):
    try:
        resultado = evaluar_con_casos_directos(code_path, casos_path)
        ...
        logger.error(f"Error al procesar la tarea {respuesta_id}: {str(e)}")

def add_task_to_queue(code_path, casos_path, respuesta_id):
    q = Queue(connection=redis.Redis(host='redis', port=6379))
    task_id = q.enqueue(procesar_tarea, code_path, casos_path, respuesta_id).id
    return task_id
