...
def menu_alumno(request):
    ...
    ejercicios_vencidos = [ejercicio.id for ejercicio in ejercicios if ejercicio.fecha_entrega < timezone.now()]
    return render(request, 'menu_alumno.html', {'ejercicios': ejercicios, 'ejercicios_vencidos': ejercicios_vencidos})
...
def subir_respuesta(request, ejercicio_id):
    ...
    # Verificar si la fecha actual es posterior a la fecha de entrega
    if timezone.now() > ejercicio.fecha_entrega:
        messages.error(request, 'La fecha de entrega ha pasado. No puedes subir respuestas para este ejercicio.')
        return redirect('menu_alumno')  # Redirigir al menú del alumno
