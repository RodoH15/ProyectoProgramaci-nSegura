...
cola = Queue('default', connection=redis_conn)
...
def worker(code_path, casos_path, respuesta_id):
    try:
        logger.debug(f"Ejecutando comando: docker exec -i programacion-programacion-executor-1-1 python3 /code/programacion/scripts/scripts.py {code_path} {casos_path}")
        result = evaluar_con_casos_directos(code_path, casos_path)
        logger.debug(f"Resultado de la evaluación: {result}")
        puntaje = calcular_puntaje(result)
        logger.debug(f"Puntaje calculado: {puntaje}")
...
def add_task_to_queue(code_path, casos_path, respuesta_id):
    job = cola.enqueue(worker, code_path, casos_path, respuesta_id)
    return job.id
