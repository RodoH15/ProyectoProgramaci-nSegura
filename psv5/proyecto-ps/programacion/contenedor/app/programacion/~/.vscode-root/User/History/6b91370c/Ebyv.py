...
from .scripts import inyect, evaluar, analizar_codigo, evaluar_con_casos_directos
...
from queue import Queue
from threading import Thread
from .scripts import analizar_codigo, evaluar_con_casos_directos, inyectar
import logging
...
def worker():
    while True:
        ...
        try:
            ...
            logger.debug(f"Resultados de evaluación: {result}")
...
def calcular_puntaje(resultados):
    total_correctos = sum(1 for _, score in resultados if score == 1)
    total_cases = len(resultados)
    puntaje_final = (total_correctos / total_cases) * 100 if total_cases > 0 else 0
    logger.debug(f"Puntaje final calculado: {puntaje_final}")
