    ...
    for line in arCasos.splitlines():
        ...
        if line == INPUT_BREAK:
            ...
            if salida[1] != 0:
                ...
                # Dividir las salidas esperadas y obtenidas en líneas
                expected_lines = salida_esperada_limpia.split('\n')
                obtained_lines = salida_obtenida_limpia.split('\n')
                
                # Log detallado de las líneas esperadas y obtenidas
                logger.debug(f"Salidas esperadas: {expected_lines}")
                logger.debug(f"Salidas obtenidas: {obtained_lines}")

                # Evaluar cada línea individualmente
                correct_lines = 0
                total_lines = max(len(expected_lines), len(obtained_lines))
                for expected, obtained in zip(expected_lines, obtained_lines):
                    logger.debug(f"Comparando: '{expected}' == '{obtained}'")
                    if expected == obtained:
                        correct_lines += 1
                    else:
                        logger.debug(f"Diferencia encontrada: Esperado: '{expected}', Obtenido: '{obtained}'")
                
                # Calcular el porcentaje de líneas correctas
                if total_lines > 0:
                    score = (correct_lines / total_lines) * 100
                else:
                    score = 0
                
                res.append((salida_obtenida_limpia, score))
