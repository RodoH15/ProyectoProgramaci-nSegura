        ...
        if form.is_valid():
            ...
            try:
                ...
                if password_valido(password, user.password):
                    # Invalida cualquier sesión existente (opcional)
                    auth_logout(request)
...
import json
import logging
from .scripts import inyect, evaluar, analizar_codigo, evaluar_con_casos_directos, calcular_puntaje
...
def worker():
    while True:
        ...
        try:
            result = evaluar_con_casos_directos(code_path, casos_path)
            puntaje = calcular_puntaje(result)
            actualizar_puntaje_respuesta(respuesta_id, puntaje, result)
            logger.info(f"Tarea {respuesta_id} procesada exitosamente. Puntaje: {puntaje}")
        except Exception as e:
            logger.error(f"Error al procesar la tarea {respuesta_id}: {e}")
        task_queue.task_done()
...
# Función para actualizar el puntaje de la respuesta
def actualizar_puntaje_respuesta(respuesta_id, puntaje, detalles):
    respuesta = RespuestaEjercicio.objects.get(id=respuesta_id)
    respuesta.puntaje = puntaje
    respuesta.detalles_evaluacion = json.dumps(detalles)  # Asegúrate de tener este campo en el modelo
    respuesta.save()
# views.py
...
from .scripts import evaluar_con_casos_directos, calcular_puntaje
import logging
import os
        ...
        if form.is_valid():
            ...
            if respuesta.codigo_archivo:
                ...
                try:
                    ...
                    respuesta.puntaje = puntaje
                    respuesta.save()
                    messages.success(request, f'La respuesta se ha subido exitosamente y ha sido evaluada. Puntaje: {puntaje}')
                except Exception as e:
                    logger.error(f"Error al evaluar la respuesta: {e}")
                    messages.error(request, f'Error al evaluar la respuesta: {e}')
...
def ver_resultado(request, respuesta_id):
    ...
    detalles = json.loads(respuesta.detalles_evaluacion)
    return render(request, 'ver_resultado.html', {'respuesta': respuesta, 'detalles': detalles})
