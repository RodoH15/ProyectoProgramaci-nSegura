...
from .models import RespuestaEjercicio, Ejercicio
...
def evaluar_con_casos_directos(code_path, casos_path):
    ...
    container = client.containers.get('programacion-programacion-executor-1-1')
    result = container.exec_run(comando)
    ...
    try:
        ...
        raise Exception(f"Error al decodificar JSON: {str(e)} - stdout: {result.output}")

def calcular_puntaje(resultados):
    total = len(resultados)
    correctos = sum(1 for res in resultados if res is True)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if 'codigo_archivo' in request.FILES:
                ...
                container = client.containers.get('programacion-programacion-executor-1-1')
                with open(archivo_path, 'rb') as filedata:
                    container.put_archive('/code/submissions', filedata)
                with open(casos_path, 'rb') as filedata:
                    container.put_archive('/code/submissions', filedata)
